...
Models come from each endpoint's `GET /models` list. It is cached in `~/.prime/agent/endpoints/` and refreshed in the background when a session starts, once it is more than an hour old. Context size, output limit, image input, reasoning and tool support are read when the endpoint reports them, as Chutes, OpenRouter and vLLM do. Reasoning is taken only from what the endpoint states (`supports_reasoning`, `reasoning: true`, `reasoning: { supported: true }` or a listed `reasoning` capability), never from model names, and an explicit `false` wins.
